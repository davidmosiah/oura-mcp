...
process.env.OURA_NO_CACHE = 'true';
{
  let calls = 0;
  const inner = async () => { calls += 1; return okResponse(); };
  const opts = { innerFetch: inner, defaultTtlSeconds: 10, envVarBypass: 'OURA_NO_CACHE' };

  await fetchWithCache('https://api.example/v1/bypass', { method: 'GET' }, opts);
  await fetchWithCache('https://api.example/v1/bypass', { method: 'GET' }, opts);
  assert.equal(calls, 2, 'OURA_NO_CACHE=true must skip cache reads AND writes');
  const stats = getCacheStats();
  assert.equal(stats.size, 0, 'env bypass must not populate cache');
}
delete process.env.OURA_NO_CACHE;
