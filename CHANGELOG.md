...
- Added `oura_agent_manifest` and `oura://agent-manifest` for machine-readable agent installation/runtime guidance.
- Added Hermes-specific diagnostics with `doctor --client hermes` and optional `client` support in `oura_connection_status`.
...
- Added anti-friction guidance for Hermes: use `/reload-mcp` or `hermes mcp test oura`, not gateway restart, for normal Oura MCP config/data access.
...
- Added OAuth scope diagnostics to `doctor` and `oura_connection_status`.
