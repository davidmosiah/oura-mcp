...
function printDoctor(status: Awaited<ReturnType<typeof buildConnectionStatus>>): void {
  ...
  console.log(`- Oura env vars: ${status.missing_env.length === 0 ? "ok" : `missing ${status.missing_env.join(", ")}`}`);
  ...
  if (status.client_checks?.hermes) {
    ...
    console.log(`  configured: ${hermes.oura_server_configured ? "ok" : "missing"}`);
