...
import { writeLocalConfig, type LocalOuraConfig } from "../services/local-config.js";
...
export async function runSetupCommand(args: string[]): Promise<number> {
  ...
  const config: LocalOuraConfig = {
    OURA_CLIENT_ID: options.clientId,
    OURA_CLIENT_SECRET: options.clientSecret,
    OURA_REDIRECT_URI: options.redirectUri,
    OURA_SCOPES: DEFAULT_SCOPES.join(" "),
    OURA_PRIVACY_MODE: options.privacyMode
  };
  if (options.cache) config.OURA_CACHE = options.cache;
  if (options.tokenPath) config.OURA_TOKEN_PATH = options.tokenPath;
  if (options.cachePath) config.OURA_CACHE_PATH = options.cachePath;
  ...
  if (options.json) console.log(JSON.stringify(setupOutput, null, 2));
    ...
    console.log("Oura MCP setup saved.");
    ...
    console.log("Secrets were saved only in the local Oura MCP config file.");
  }
...
async function parseSetupOptions(args: string[]): Promise<SetupOptions> {
  ...
  const clientId = required(answers, "client-id", "Oura Client ID");
  const clientSecret = required(answers, "client-secret", "Oura Client Secret");
...
async function promptForMissing(flags: Map<string, string>): Promise<Map<string, string>> {
  ...
  try {
    ...
    if (!merged.has("client-id")) merged.set("client-id", (await firstPrompt.question("Oura Client ID: ")).trim());
  } finally {
    firstPrompt.close();
  }
  if (!merged.has("client-secret")) merged.set("client-secret", await promptHidden("Oura Client Secret: "));

  const secondPrompt = createPromptInterface({ input, output });
  try {
    if (!merged.has("redirect-uri")) merged.set("redirect-uri", (await secondPrompt.question("Oura Redirect URI [http://127.0.0.1:3000/callback]: ")).trim() || "http://127.0.0.1:3000/callback");
...
function writeClientConfig(client: AgentClientName, homeDir: string): ClientConfigResult {
  ...
  const path = join(homeDir, ".oura-mcp", "mcp-configs", `${client}.json`);
...
function mergeClaudeConfig(homeDir: string): string {
  const path = process.platform === "darwin"
    ...
    : join(homeDir, ".oura-mcp", "mcp-configs", "claude-desktop.json");
  mkdirSync(dirname(path), { recursive: true, mode: 0o700 });
  ...
  const next = {
    ...
    mcpServers: {
      ...mcpServers,
      oura: mcpConfigSnippet().mcpServers.oura
    }
  };
...
function writeHermesClientConfig(homeDir: string): ClientConfigResult {
  ...
  return {
    ...
    warnings: [
      "After editing Hermes MCP config, use `/reload-mcp` or `hermes mcp test oura`; do not restart the Hermes gateway for normal Oura data access.",
...
function mergeHermesConfig(configPath: string): string | undefined {
  ...
  if (/oura-mcp-unofficial|oura-mcp-server|oura-mcp/.test(existing) && /^\s*oura\s*:/m.test(existing)) {
    if (existing.includes(PINNED_NPM_PACKAGE)) return undefined;
    const backupPath = backupConfig(configPath);
    const updated = existing.replace(/oura-mcp-unofficial(?:@\d+\.\d+\.\d+)?/g, PINNED_NPM_PACKAGE);
  ...
  const backupPath = backupConfig(configPath);
  const next = existing.trimEnd().length ? addHermesOuraBlock(existing) : snippet;
  writeFileSync(configPath, ensureReloadHint(next), { mode: 0o600 });
  chmodSync(configPath, 0o600);
  return backupPath;
}

function addHermesOuraBlock(existing: string): string {
  const serverBlock = [
    "  oura:",
    "    command: npx",
    "    args:",
...
function backupConfig(path: string): string {
  const backupPath = `${path}.bak-oura-mcp-${new Date().toISOString().replace(/[-:]/g, "").replace(/\..+/, "Z")}`;
...
function setupNextStep(client: AgentClientName, noAuth: boolean): string {
  const auth = noAuth ? "Run `oura-mcp-server auth`, then " : "";
  if (client === "hermes") {
    return `${auth}run \`oura-mcp-server doctor --client hermes\`, then use \`/reload-mcp\` or \`hermes mcp test oura\`.`;
  }
  return `${auth}run \`oura-mcp-server doctor\`.`;
}
