...
export type LocalOuraConfig = Partial<Record<LocalConfigKey, string>>;

export interface LocalConfigReadResult {
  path: string;
  exists: boolean;
  values: LocalOuraConfig;
  permissions?: string;
...
export function getLocalConfigPath(homeDir = homedir()): string {
  return join(homeDir, ".oura-mcp", "config.json");
}
...
export function writeLocalConfig(values: LocalOuraConfig, homeDir = homedir()): string {
  const path = getLocalConfigPath(homeDir);
